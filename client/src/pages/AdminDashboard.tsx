  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="container flex h-16 items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><button type="button" className="grid h-10 w-10 place-items-center rounded-xl text-slate-600 transition hover:bg-slate-100 md:hidden" aria-label="Administrator navigation"><PanelLeft className="h-5 w-5" /></button><Link href="/admin" className="flex min-w-0 items-center gap-2"><div className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-600 text-white shadow-sm"><Shield className="h-5 w-5" /></div><div className="min-w-0"><p className="truncate text-sm font-extrabold tracking-[-0.03em] text-slate-950">Nyumba 360</p><p className="hidden text-[10px] font-bold uppercase tracking-[0.12em] text-emerald-700 sm:block">Admin console</p></div></Link></div><div className="flex items-center gap-2"><button type="button" onClick={() => setActiveTab("moderation")} className="relative grid h-10 w-10 place-items-center rounded-xl text-slate-600 transition hover:bg-amber-50 hover:text-amber-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600" aria-label={`${pendingReviewCount} ${pendingReviewCount === 1 ? "property" : "properties"} pending human review. Open moderation queue.`}><ShieldAlert className="h-5 w-5" /><span aria-hidden="true" className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-amber-500 px-1 text-[10px] font-extrabold leading-5 text-amber-950 ring-2 ring-white">{pendingReviewCount > 99 ? "99+" : pendingReviewCount}</span></button><Button variant="ghost" size="icon" className="rounded-xl text-slate-600" aria-label="Platform alerts"><Bell className="h-5 w-5" /></Button><Link href="/admin/modules" className="hidden rounded-xl px-3 py-2 text-xs font-bold text-emerald-700 transition hover:bg-emerald-50 sm:block">Module controls</Link><div className="grid h-9 w-9 place-items-center rounded-full bg-slate-950 text-xs font-extrabold text-white" title={user?.name ?? "Administrator"}>{user?.name?.slice(0, 1).toUpperCase() ?? "A"}</div></div></div>
      </header>
